...
export function extendDynamoDBWithUserMethods(): void {
  ...
  extendedDynamoDB.getUser = async (discordUserId: string): Promise<User | null> => {
    try {
      ...
      logger.error({ error: error.message, discordUserId }, "Error getting user from DynamoDB");
      return null;
    }
  };

  // Get user by verification ID - uses the flattened verificationId index
  extendedDynamoDB.getUserByVerificationId = async (verificationId: string): Promise<User | null> => {
    try {
      ...
      logger.error({ error: error.message, verificationId }, "Error getting user by verification ID");
      return null;
    }
  };

  // Get user by wallet address
  extendedDynamoDB.getUserByWalletAddress = async (walletAddress: string): Promise<User | null> => {
    try {
      ...
      logger.error({ error: error.message, walletAddress }, "Error getting user by wallet address");
      return null;
    }
  };
  ...
  extendedDynamoDB.createUser = async (user: User): Promise<User | null> => {
    try {
      ...
      if (dynamoUser.walletAddress) {
        dynamoUser.walletAddress = dynamoUser.walletAddress.toLowerCase();
      } else {
      ...
      if (dynamoUser.humanPassport) {
        ...
        for (const [key, value] of Object.entries(dynamoUser.humanPassport)) {
        ...
        if (Object.keys(cleanHumanPassport).length > 0) {
          ...
          if (cleanHumanPassport.verificationId) {
            dynamoUser.verificationId = cleanHumanPassport.verificationId;
          }
        } else {
  ...
  // Update a user
  extendedDynamoDB.updateUser = async (discordUserId: string, updates: Partial<User>): Promise<boolean> => {
    try {
      ...
      for (const [key, value] of Object.entries(updates)) {
        ...
        if (value === null) {
        ...
        } else if (key === 'humanPassport' && typeof value === 'object') {
          ...
          const cleanHumanPassport: Record<string, any> = {};
          for (const [hpKey, hpValue] of Object.entries(humanPassport)) {
            if (hpValue !== null) {
          ...
          if (cleanHumanPassport.verificationId) {
            expressionAttributeValues[`:verificationId`] = cleanHumanPassport.verificationId;
            expressionAttributeNames[`#verificationId`] = 'verificationId';
            updateExpressions.push(`#verificationId = :verificationId`);
          }
        } else {
          // Normal case
      ...
      logger.error({ error: error.message, discordUserId, updates }, "Error updating user");
      return false;
    }
  };

  // Delete a user
  extendedDynamoDB.deleteUser = async (discordUserId: string): Promise<boolean> => {
    try {
      ...
      logger.error({ error: error.message, discordUserId }, "Error deleting user");
      return false;
    }
  };
}
