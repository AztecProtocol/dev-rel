...
export async function handleStatusCommand(
	...
	try {
		...
		const session = await dynamoDB.findSessionByDiscordId(userId);
		...
		if (statusData.verified && statusData.roleAssigned) {
			if (statusData.isHighScorer) {
				embed.setDescription(
					"✅ Your wallet has been verified successfully and you've been assigned the verified and high scorer roles!"
				);
				embed.addFields({
					name: "Passport Score",
					value: `${statusData.score} (exceeds high scorer threshold of ${statusData.highScoreThreshold})`,
				});
			} else {
				embed.setDescription(
