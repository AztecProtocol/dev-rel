...
import { RollupAbi, ForwarderAbi, StakingAssetHandlerAbi } from "@sparta/utils";
import { ForwarderBytecode } from "@sparta/utils";
...
export class Ethereum {
	constructor(
		...
		private rollup: any,
		private stakingAssetHandler: any
	) {}
	...
	getStakingAssetHandler = () => {
		return this.stakingAssetHandler;
	};

	// TODO: For now, the withdrawer address is managed by the bot
	stakingAssetFaucet = async (address: string) => {
		const txHash = await this.stakingAssetHandler.write.mint([
	...
	addValidator = async (address: string): Promise<TransactionReceipt[]> => {
		...
		const hashes = await Promise.all(
			[
				await this.stakingAssetHandler.write.approve([
					this.rollup.address,
