...
class DynamoDBService {
  ...
  constructor() {
    ...
    if (this.isLocal) {
      options.endpoint = process.env.DYNAMODB_LOCAL_ENDPOINT || "http://localhost:8000";
  ...
  public async getSession(sessionId: string): Promise<Session | undefined> {
    try {
      ...
      logger.error({ error, sessionId }, "Error retrieving session from DynamoDB");
      return undefined;
    }
  }
  ...
  public async updateSession(
    sessionId: string,
    updates: Partial<Omit<Session, "sessionId" | "discordUserId" | "createdAt">>
  ): Promise<boolean> {
    try {
      ...
      logger.error({ error, sessionId }, "Error updating session in DynamoDB");
      return false;
    }
  }
   ...
   * @returns True if successful, false otherwise.
   */
  public async updateSignature(sessionId: string, signature: string): Promise<boolean> {
  ...
  public async createSession(
    ...
    try {
      ...
      logger.debug({ sessionId }, "No existing session found, creating new one");
      ...
      logger.debug({ sessionId, tableName: this.tableName }, "Sending PutCommand to DynamoDB");
      await this.client.send(command);
      logger.info({ sessionId, discordUserId }, "Created new verification session in DynamoDB.");
      return newSession;
    } catch (err: any) {
   ...
   */
  public async findSessionByDiscordId(discordUserId: string): Promise<Session | undefined> {
    try {
      ...
      const validSessions = sessions.filter(
        session => Date.now() - session.createdAt <= SESSION_TIMEOUT_MS
      );
      ...
      logger.error({ error, discordUserId }, "Error finding session by Discord ID in DynamoDB");
      return undefined;
    }
  }
