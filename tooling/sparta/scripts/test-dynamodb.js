...
const tableName = process.argv[2] || 'sparta-sessions';
...
async function runTest() {
  try {
    ...
    console.log(`   Found ${scanResponse.Items?.length || 0} items in the table`);
    if (scanResponse.Items?.length > 0) {
      console.log('   Sample item:', JSON.stringify(scanResponse.Items[0], null, 2));
    }
